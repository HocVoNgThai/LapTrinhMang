...
try
{
    ...
    var monAnElement = doc.GetElementbyId("monAn"); // Assuming an ID for monAn (optional)
    if (monAnElement != null)
    {
        lbMonAn.Text = monAnElement.InnerText;
    }
    var diaChiElement = doc.GetElementbyId("diaChi"); // Assuming an ID for diaChi (optional)
    if (diaChiElement != null)
    {
        lbDiaChi.Text = diaChiElement.InnerText;
    }
    var giaElement = doc.GetElementbyId("Gia"); // Assuming an ID for diaChi (optional)
    if (giaElement != null)
    {
        lbGia.Text = giaElement.InnerText;
    }
    if (doc.DocumentNode.Name == "img")
    {
        string sourceUrl = doc.DocumentNode.GetAttributeValue("src", null);
        pbHinhAnh.ImageLocation = sourceUrl;
    }
///html/body/p/img
